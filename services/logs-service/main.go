...
func getLogs(c *fiber.Ctx) error {
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Failed to fetch logs from Redis"})
	}
...
func sendToLoki(logEntry LogEntry) error {
	...
	// Send to Loki
	resp, err := fiber.Post(lokiURL).Body(payloadBytes).Send()
	if err != nil {
